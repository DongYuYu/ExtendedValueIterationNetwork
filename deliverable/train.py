...
def train(net, trainloader, config, criterion, optimizer, use_GPU):
    ...
    for epoch in range(config.epochs): # Loop over dataset multiple times
        ...
        for i, data in enumerate(trainloader): # Loop over batches of data
            ...
            S2, O1 = Variable(S2), Variable(O1)
            O2, labels = Variable(O2), Variable(labels)
            # Zero the parameter gradients
            optimizer.zero_grad()
            # Forward pass
            if record == True:
                outputs, predictions = net(X, S1, S2, O1, O2, config, record_images=True)
                imgs = np.concatenate([net.grid_image] + [net.reward_image] + net.value_images)
                np.savez_compressed('learned_rewards_values_{:d}x{:d}'.format(config.imsize, config.imsize), imgs)

                record = False

#            print("train")
#            print(X.shape)
            outputs, predictions = net(X, S1, S2, O1, O2, config)
 #           print("put")
 #           print(outputs)
  #          print("labels")
  #          print(labels)
...
def test(net, testloader, config):  
    ...
    epoch = 0
    avg_loss, avg_error, num_batches= 0.0, 0.0, 0.0
    start_time = time.time()
    for i, data in enumerate(testloader):
        ...
        if use_GPU:
            ...
            labels = labels.cuda()
        # Wrap to autograd.Variable
        X, S1, S2, O1, O2 = Variable(X), Variable(S1), Variable(S2), Variable(O1), Variable(O2)
        # Forward pass
        outputs, predictions = net(X, S1, S2, O1, O2, config)
        loss = criterion(outputs, labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datafile', 
                        type=str, 
                        default='dataset/gridworld1000_8x8.npz', 
    ...
    save_path = "trained/vint_{0}x{0}.pth".format(config.imsize) 
   ...
   # s = SubsetRandomSampler()
    ...
    # Create Dataloader
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=config.batch_size, sampler= SubsetRandomSampler(list(range(700,701))), shuffle=False, num_workers=0)
