...
#                print(ns)
#                print("shape")
#                print(states_xy[i].shape[0])
#                print("leng")
#                print(len(states_xy[i]))
                ...
                value_data = np.resize(value[i][0], (1,1,dom_size[0],
                                                         dom_size[1]))
                iv_mixed = np.concatenate((image_data, value_data), axis=1)
                
                X_current = np.array(iv_mixed)
 ...
 #                       print("lengv")
 #                       print(len(value))
                        value_data = np.resize(value[i][j], (1,1,dom_size[0],
                                                         dom_size[1]))
                        iv_mixed = np.concatenate((image_data, value_data), axis=1)
                        X_current = np.append(X_current, iv_mixed, axis = 0)
                    
                #print(iv_mixed.shape)
                ...
                # Resize statesO
                O1_current = np.expand_dims(stateO[i][0:ns, 0], axis=1)
                O2_current = np.expand_dims(stateO[i][0:ns, 1], axis=1)
                # Resize labels
                ...
                print(X_current)
                print("s1")
                print(S1_current.shape)
                print("label")
                print(Labels_current.shape)
                X_l.append(X_current)
                S1_l.append(S1_current)
                S2_l.append(S2_current)
                O1_l.append(O1_current)
                O2_l.append(O2_current)
...
#    X_f =[]    
    ...
    O1_f = np.concatenate(O1_l)
    O2_f = np.concatenate(O2_l)
    Labels_f = np.concatenate(Labels_l)
    return X_f, S1_f, S2_f, O1_f, O2_f, Labels_f


def main(dom_size=[16,16], n_domains=1, max_obs=30, max_obs_size=None, 
            n_traj=1, state_batch_size=1):
    # Get path to save dataset
    save_path = "dataset/gridworldt_{0}x{1}".format(dom_size[0], dom_size[1])
    # Get training data
    print("Now making training data...")    
    X_out_tr, S1_out_tr, S2_out_tr, O1_out_tr, O2_out_tr, Labels_out_tr = make_data(
        dom_size, n_domains, max_obs, max_obs_size, n_traj, state_batch_size)
    # Get testing data
    print("\nNow making  testing data...")
    X_out_ts, S1_out_ts, S2_out_ts, O1_out_ts, O2_out_ts, Labels_out_ts = make_data(
        dom_size, n_domains/6, max_obs, max_obs_size, n_traj, state_batch_size)
    # Save dataset
    np.savez_compressed(save_path, X_out_tr, S1_out_tr, S2_out_tr,O1_out_tr, O2_out_tr, 
                        Labels_out_tr, X_out_ts, S1_out_ts, S2_out_ts, O1_out_ts, O2_out_ts, Labels_out_ts)
